  ...
  class ScalingPolicy
    ...
    def self.openapi_types
      {
        ...
        :'namespace' => :'String',
        :'policy' => :'Hash<String, AnyType>',
        :'target' => :'Hash<String, String>',
        :'type' => :'String'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NomadClient::ScalingPolicy` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NomadClient::ScalingPolicy`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'policy')
        if (value = attributes[:'policy']).is_a?(Hash)
          self.policy = value
        end
      end
    ...
    def hash
      [create_index, enabled, id, max, min, modify_index, namespace, policy, target, type].hash
    end
