  ...
  class PeriodicForceResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NomadClient::PeriodicForceResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NomadClient::PeriodicForceResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@index.nil? && @index > 384
        invalid_properties.push('invalid value for "index", must be smaller than or equal to 384.')
      end
    ...
    def valid?
      ...
      return false if !@index.nil? && @index > 384
    ...
    def index=(index)
      if !index.nil? && index > 384
        fail ArgumentError, 'invalid value for "index", must be smaller than or equal to 384.'
      end
    ...
    def hash
      [eval_create_index, eval_id, index].hash
    end
