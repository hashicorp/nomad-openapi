...
module NomadClient
  class CSINodeInfo
    attr_accessor :accessible_topology

    attr_accessor :id

    attr_accessor :max_volumes
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NomadClient::CSINodeInfo` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NomadClient::CSINodeInfo`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'accessible_topology')
        self.accessible_topology = attributes[:'accessible_topology']
      end
      ...
      if attributes.key?(:'max_volumes')
        self.max_volumes = attributes[:'max_volumes']
      end
