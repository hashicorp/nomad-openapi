...
module NomadClient
  class JobDispatchResponse
    attr_accessor :dispatched_job_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NomadClient::JobDispatchResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NomadClient::JobDispatchResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'dispatched_job_id')
        self.dispatched_job_id = attributes[:'dispatched_job_id']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          dispatched_job_id == o.dispatched_job_id &&
    ...
    def hash
      [dispatched_job_id, eval_create_index, eval_id, job_create_index, last_index, request_time].hash
    end
