...
module NomadClient
  class ConsulProxy
    ...
    attr_accessor :local_service_port
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NomadClient::ConsulProxy` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NomadClient::ConsulProxy`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'expose_config')
        self.expose_config = attributes[:'expose_config']
      end

      if attributes.key?(:'local_service_address')
        self.local_service_address = attributes[:'local_service_address']
      end

      if attributes.key?(:'local_service_port')
        self.local_service_port = attributes[:'local_service_port']
      end
