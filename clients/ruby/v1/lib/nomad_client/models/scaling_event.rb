  ...
  class ScalingEvent
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NomadClient::ScalingEvent` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NomadClient::ScalingEvent`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'previous_count')
        self.previous_count = attributes[:'previous_count']
      end
    ...
    def list_invalid_properties
      ...
      if !@time.nil? && @time > 384
        invalid_properties.push('invalid value for "time", must be smaller than or equal to 384.')
      end
    ...
    def valid?
      ...
      return false if !@time.nil? && @time > 384
    ...
    def time=(time)
      if !time.nil? && time > 384
        fail ArgumentError, 'invalid value for "time", must be smaller than or equal to 384.'
      end
    ...
    def hash
      [count, create_index, error, eval_id, message, meta, previous_count, time].hash
    end
