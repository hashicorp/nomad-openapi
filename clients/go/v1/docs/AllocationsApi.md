...
> Allocation GetAllocation(ctx, allocationID).Region(region).Namespace(namespace).Index(index).Wait(wait).Stale(stale).Prefix(prefix).XNomadToken(xNomadToken).PerPage(perPage).NextToken(nextToken).Execute()
...
func main() {
    allocationID := "allocationID_example" // string | Specifies the UUID of the allocation. This must be the full UUID, not the short 8-character one. This is specified as part of the path.
    ...
    resp, r, err := api_client.AllocationsApi.GetAllocation(context.Background(), allocationID).Region(region).Namespace(namespace).Index(index).Wait(wait).Stale(stale).Prefix(prefix).XNomadToken(xNomadToken).PerPage(perPage).NextToken(nextToken).Execute()
    if err != nil {
...
> AllocStopResponse StopAllocation(ctx, allocationID).Region(region).Namespace(namespace).XNomadToken(xNomadToken).IdempotencyToken(idempotencyToken).Execute()
...
func main() {
    ...
    configuration := openapiclient.NewConfiguration()
    api_client := openapiclient.NewAPIClient(configuration)
    resp, r, err := api_client.AllocationsApi.StopAllocation(context.Background(), allocationID).Region(region).Namespace(namespace).XNomadToken(xNomadToken).IdempotencyToken(idempotencyToken).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `AllocationsApi.StopAllocation``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `StopAllocation`: AllocStopResponse
    fmt.Fprintf(os.Stdout, "Response from `AllocationsApi.StopAllocation`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiStopAllocationRequest struct via the builder pattern
