...
> [ScalingPolicyListStub] getScalingPolicies(opts)
...
let apiInstance = new nomad-client.ScalingApi();
let opts = {
...
apiInstance.getScalingPolicies(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
> ScalingPolicy getScalingPolicy(policyID, opts)
...
let apiInstance = new nomad-client.ScalingApi();
let policyID = "policyID_example"; // String | The scaling policy identifier.
let opts = {
  'region': "region_example", // String | Filters results based on the specified region.
  'namespace': "namespace_example", // String | Filters results based on the specified namespace.
  'index': 56, // Number | If set, wait until query exceeds given index. Must be provided with WaitParam.
...
apiInstance.getScalingPolicy(policyID, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
